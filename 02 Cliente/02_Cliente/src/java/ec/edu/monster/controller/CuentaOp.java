...
public class CuentaOp {
    ...
    public ArrayList<Cuenta> getCuentasDeUsuario(String cedula) throws Exception {
        String test = peticionHttpGet("http://localhost:8080/01_Server_CoreBancario/webresources/Core/cedula/" + cedula);

        Gson gson = new Gson();
        Type listType = new TypeToken<ArrayList<Cuenta>>() {
        }.getType();
        ArrayList<Cuenta> arrayDeJson = gson.fromJson(test, listType);
